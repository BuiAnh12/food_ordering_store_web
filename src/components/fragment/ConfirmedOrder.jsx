            ...
            <div className="w-full p-4 bg-gray-50">
                ...
                <div className="bg-white p-4 rounded-lg shadow-md mb-4">
                    <ul className="mb-4 text-sm text-gray-700">
                        {order?.items?.map((item) => (
                            <li key={item._id} className="flex justify-between py-2">
                                ...
                                <span>{item.dish.price.toLocaleString()}₫</span>
                            </li>
                        ))}
                    </ul>
                    ...
                    <div className="text-sm">
                        <div className="flex justify-between mb-2">
                            ...
                            <span className="text-gray-800 font-medium">{order?.totalDiscount}₫</span>
                        </div>
                        <div className="flex justify-between mb-2">
                            <span className="text-gray-700">Phí giao hàng</span>
                            <span className="text-gray-800 font-medium">{order?.shippingFee}₫</span>
                        </div>
                        {/* <div className="flex justify-between mb-2">
                        ...
                        <div className="flex justify-between mb-2">
                            ...
                            <span className="text-gray-800 font-medium">{order?.subtotalPrice}₫</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-md font-bold">Tổng tiền quán nhận</span>
                            <span className="text-md font-bold text-[#fc6011]">{order?.finalTotal}₫</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-lg shadow-md">
                    ...
                    <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="text-gray-800">{new Date(order?.createdAt).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit", hour12: false })}</span>
                    </div>
                </div>
                <div className="flex w-full space-x-4 justify-between my-4 p-1">
                    ...
                    <button className="py-2 px-4 bg-[#fc6011] text-md text-white rounded-md hover:bg-[#e9550f] w-full" onClick={() => router.push(`/orders/${order._id}/update`)}>
                        Sửa
                    </button>
                </div>
            </div>
