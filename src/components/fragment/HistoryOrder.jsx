    ...
    <div className="w-full px-4 py-2 mt-20 mb-20">
      <div className="w-full p-4 bg-gray-50">
        ...
        <div className="flex items-center bg-white p-4 rounded-lg shadow-md mb-4">
          <div className="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center"></div>
          <div className="ml-3">
            <h3 className="text-gray-800 font-medium">{order?.driver || "Chưa có tài xế"}</h3>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow-md mb-4">
          <ul className="mb-4 text-sm text-gray-700 space-y-3">
            {order?.items?.map((item, index) => {
              const basePrice = item?.dish?.price ?? item?.price ?? 0;
              ...
              return (
                <li key={item?._id || index} className="py-2 border-b last:border-b-0">
                  <div className="flex justify-between">
                    <span>
                      {item?.quantity} x {item?.dish?.name || item?.dishName}
                    </span>
          ...
          <div className="text-sm">
            ...
            <div className="flex justify-between mb-2">
              ...
              <span className="text-gray-800 font-medium">
                {formatVND(order?.subtotalPrice ?? computed.subtotal)}
              </span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="bg-white p-4 rounded-lg shadow-md">
          <div className="flex justify-between text-sm mb-2">
            ...
            <a href="/" className="text-blue-500 underline">{order?._id || "N/A"}</a>
          </div>
          <div className="flex justify-between text-sm mb-2">
          ...
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="text-gray-800">{order?.metadata?.distance || "N/A"}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">Thời gian lấy hàng dự kiến</span>
            <span className="text-gray-800">{order?.metadata?.estimatedPickup || "N/A"}</span>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow-md mt-2">
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="text-gray-800">{order?.status || "Chưa cập nhật"}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">Phương thức thanh toán</span>
            <span className="text-gray-800">{order?.paymentMethod || "Không xác định"}</span>
          </div>
        </div>
