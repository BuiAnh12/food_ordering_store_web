...
const VerifyOrderTab = ({ storeId }) => {
  ...
  const fetchOrders = useCallback(async () => {
    setIsLoading(true);
    try {
      const statuses = selectedStatus === "all" ? ["confirmed", "finished"] : [selectedStatus];
      ...
      setOrders(res.data?.toSorted((a, b) => new Date(a.createdAt) - new Date(b.createdAt)) || []);
  ...
  return (
    <div className='w-full px-4 py-2'>
      <Dropdown options={dropdownOptions} onChange={handleDropdownChange} value={selectedStatusLabel} />
      {isLoading ? (
        ...
        <>
          ...
          <div className='flex items-center justify-center w-full h-max mt-10 mb-20'>
            <ReactPaginate
              previousLabel={
                <svg className='w-5 h-5' stroke='currentColor' fill='none' viewBox='0 0 24 24' strokeWidth={2}>
